...
var { smsg, getGroupAdmins, formatp, tanggal, formatDate, getTime, isUrl, sleep, clockString, runtime, fetchJson, getBuffer, jsonformat, delay, format, logic, generateProfilePicture, parseMention, getRandom, yts, ytdl } = require('../message/myfunc')
...
const ftroli ={key: {fromMe: false,"participant":"0@s.whatsapp.net", "remoteJid": "6289523258649-1604595598@g.us"}, "message": {orderMessage: {itemCount: 2021,status: 200, thumbnail: fs.readFileSync('./image/zeroyt7.jpg'), surface: 200, message: `© ZaBot`, orderTitle: pushname, sellerJid: '0@s.whatsapp.net'}}, contextInfo: {"forwardingScore":999,"isForwarded":true},sendEphemeral: true}
...
var api = await yts(text)
menuBut = [{index: 1,quickReplyButton: {displayText: 'audio',id: `.ytmp3 https://youtube.com/watch?v=${api.id}`}}, {index: 2,quickReplyButton: {displayText: 'video',id: `.ytmp4 https://youtube.com/watch?v=${api.id}`}}]
hh = await getBuffer(api.thumbnail.url)
await zeroyt7.sendMessage(m.chat, { caption: `»▬▬«•●•»▬▬«
*_${api.title}_*
_${api.uploadedAt}_`,location: {jpegThumbnail: hh},templateButtons: menuBut,footer: pushname},)
break
case 'ytmp3':
case 'mp3':
...
if (api.mp3.size.replace(' MB', '') < 55) {
  m.reply(`downloading ${api.mp3.size}`)
  buf = await getBuffer(api.mp3.link)
  zeroyt7.sendMessage(m.chat, {document:buf,mimetype:'audio/mpeg',fileName:`${api.title}.mp3`},{quoted:m})
} else if (api.mp3.size.replace(' KB', '') < 1024) {
  m.reply(`downloading ${api.mp3.size}`)
  buf = await getBuffer(api.mp3.link)
  zeroyt7.sendMessage(m.chat, {document:buf,mimetype:'audio/mpeg',fileName:`${api.title}.mp3`},{quoted:m})
} else {
  m.reply('file over 55 MB')
}
break
case 'ytmp4':
case 'mp4':
...
if (api.mp4.size.replace(' MB', '') < 55) {
  m.reply(`downloading ${api.mp4.size}`)
  buf = await getBuffer(api.mp4.link)
  zeroyt7.sendMessage(m.chat, {video:buf},{quoted:m})
} else if (api.mp4.size.replace(' KB', '') < 1024) {
  m.reply(`downloading ${api.mp4.size}`)
  buf = await getBuffer(api.mp4.link)
  conn.sendMessage(m.chat, {video:buf},{quoted:m})
} else {
  m.reply('file over 55 MB')
}
break
